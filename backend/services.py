...
# The fee/quote math (deal_money and everything built on it) moved to its own
# module, backend/money.py — task #30. Re-exported here (rather than every
# router switching its import) so every existing `from .services import
# deal_money_for` etc. keeps working unchanged; services.py is still where
# the *committing* siblings of these functions live (create_deal_payout,
# settle_pool_deal, settle_affiliate_program — the ones that actually call
# Stripe and write to the DB), and those call straight into money.py for the
# numbers rather than recomputing anything.
from .money import (  # noqa: F401 (re-exported for existing call sites)
    affiliate_settlement_for, deal_money, deal_money_for, deal_quote,
    pool_settlement_for, total_charge_for,
)
