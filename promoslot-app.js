...
function renderListingModal(l,tab){
  ...
  if(tab==="offers"){
    ...
    <div class="det-sec"><h5>Offers & pricing — set by ${esc(l.owner.split(" ")[0])}</h5>
      ...
      <div class="note blue">🔒 Payment is funded into PromoSlot escrow before work starts and released only when the agreed delivery conditions are verified. PromoSlot deducts 20% from the completed transaction.</div></div>`;
  } else if(tab==="about"){
...
function renderDeal(id){
  ...
  if(d.step===1){
    ...
    const roadmap = `<div class="det-sec" style="margin-top:24px">
      ...
      ${lockedStep("💸","Payout (minus 20% fee)","After a reviewer verifies delivery, funds transfer to the owner via Stripe Connect, minus PromoSlot's 20% fee. Released only on a real successful transfer. Payouts are not connected yet.")}
    </div>`;
    ...
    <div class="deal-grid">
      ...
      <div class="deal-side">
        ...
        <div class="side-card trust-card"><h5>Protected by PromoSlot</h5>
          <p>Escrow-secured funds · verified delivery · dispute support · 20% fee only on completion.</p></div>
      </div>
    </div>`;
}
  ...
  $("platDash").innerHTML=`
    ...
    <div class="dash-cols"><div>
      ...
      <div class="panel"><div class="panel-h"><h4>Payout settings</h4></div><div class="panel-b mini-rows">
        ...
        <div><span>Marketplace fee</span><b>20% on completed deals</b></div>
...
const NAV_ACTIONS={
  ...
  "toast-fees":()=>toast("Fees: 20% per completed transaction. No listing fees.")
};
function PSBoot(){
  renderMiniMarket();
  syncNav();
